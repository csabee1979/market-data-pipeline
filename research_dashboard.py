...
def main():
    ...
    # Create main header
    st.markdown('<h1 class="main-header">🔬 AI Research Papers Analytics</h1>', 
...
def show_overview_page(filters: dict):
    ...
    with col3:
        st.metric(
            ...
            value=format_number(metrics.get('total_authors', 0))
        )
    ...
    with col2:
        ...
        if not topics_df.empty:
            ...
            ai_percentage = (ai_papers / total_topic_papers * 100) if total_topic_papers > 0 else 0
    ...
    if not recent_papers.empty:
        ...
        search_term = st.text_input("🔍 Search papers by title:", placeholder="Enter keywords...")
        ...
        if not display_df.empty:
            ...
            display_papers['date'] = pd.to_datetime(display_papers['date']).dt.strftime('%Y-%m-%d')
            display_papers['open_access'] = display_papers['open_access'].map({True: '✅ Yes', False: '❌ No'})
            display_papers['title'] = display_papers['title'].str.slice(0, 80) + '...'
            
            # Display as interactive table
            st.dataframe(
                display_papers[['title', 'journal', 'date', 'citations', 'open_access', 'country']],
                column_config={
                    "title": "Paper Title",
                    "journal": "Journal",
                    "date": "Published",
...
def show_publications_page(filters: dict):
    ...
    if not journals_df.empty:
        ...
        with col2:
            ...
            display_journals = display_journals.round({'avg_citations': 1, 'oa_percentage': 1})
            
            st.dataframe(
                display_journals[['journal', 'paper_count', 'avg_citations', 'oa_percentage']],
                column_config={
    ...
    if not geo_df.empty:
        ...
        with col2:
            ...
            if 'institution' in authors_df.columns:
                inst_counts = authors_df.groupby('institution').agg({
                    'papers': 'sum',
                    'citations': 'sum'
                }).reset_index().sort_values('citations', ascending=False)
                
                inst_counts = inst_counts[inst_counts['institution'] != 'Unknown'].head(10)
                ...
                if not inst_counts.empty:
                    fig = px.bar(
                        ...
                        labels={'citations': 'Total Citations', 'institution': 'Institution'},
                        orientation='h'
                    )
                    fig.update_layout(height=500, yaxis={'categoryorder':'total ascending'})
                    st.plotly_chart(fig, use_container_width=True)
...
def show_topics_page(filters: dict):
    ...
    with col2:
        ...
        fig = px.bar(
            ...
            labels={'avg_citations': 'Average Citations per Paper', 'topic': 'Research Topic'},
            orientation='h'
        )
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "avg_citations": st.column_config.NumberColumn("Avg Citations", format="%.1f"),
            "ai_papers": st.column_config.NumberColumn("AI Papers", format="%d"),
            "ai_percentage": st.column_config.NumberColumn("AI Focus %", format="%.1f%%")
        },
        hide_index=True,
        use_container_width=True
    )
...
def show_geographic_page(filters: dict):
    ...
    with col1:
        ...
        fig = px.scatter(
            ...
            labels={'paper_count': 'Number of Papers', 'avg_citations': 'Average Citations'}
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        geo_with_oa['oa_percentage'] = (geo_with_oa['oa_papers'] / geo_with_oa['paper_count'] * 100).round(1)
